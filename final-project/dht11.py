...
# Update      : 2017/07/06
# Url		  : http://osoyoo.com/2017/07/06/dht11/
##########################################################################
    ...
    for current in data:
        ...
        if state == STATE_DATA_PULL_DOWN:
            if current == GPIO.LOW:
                ...
                continue
    if len(lengths) != 40:
        print "error: length not equal to 40"
        return False
...
def main():
    ...
    while True:
        ...
        if result:
            ...
            print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')+": humidity: %s %%,  Temperature: %s C" % (humidity, temperature)
        time.sleep(60)
